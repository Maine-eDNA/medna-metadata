...
class TaxonPhylumSerializer(serializers.ModelSerializer):
    id = serializers.IntegerField(read_only=True)
    taxon_phylum = serializers.CharField(max_length=255, validators=[UniqueValidator(queryset=TaxonPhylum.objects.all())])
...
class TaxonClassSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TaxonClass
        fields = ['id',
                  'taxon_class_slug', 'taxon_class', 'taxon_phylum', 'taxon_phylum_slug', 'taxon_url',
    ...
    # slug to tell it to print the desired field from the other table
    created_by = serializers.SlugRelatedField(many=False, read_only=True, slug_field='email')
    taxon_phylum = serializers.SlugRelatedField(many=False, read_only=False, slug_field='taxon_phylum_slug', queryset=TaxonPhylum.objects.all())
...
class TaxonOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id',
                  ...
                  'taxon_class_slug', 'taxon_url', 'taxon_phylum_slug',
                  'taxon_kingdom_slug', 'taxon_domain_slug',
                  'created_by', 'created_datetime', 'modified_datetime', ]
    # Since project, system, watershed, and created_by reference different tables and we
    # want to show 'label' rather than some unintelligible field (like pk 1), have to add
...
class TaxonFamilySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id',
                  ...
                  'taxon_order_slug', 'taxon_url', 'taxon_class_slug', 'taxon_phylum_slug',
                  'taxon_kingdom_slug', 'taxon_domain_slug',
                  'created_by', 'created_datetime', 'modified_datetime', ]
    # Since project, system, watershed, and created_by reference different tables and we
    # want to show 'label' rather than some unintelligible field (like pk 1), have to add
...
class TaxonGenusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TaxonGenus
        fields = ['id',
                  'taxon_genus_slug', 'taxon_genus', 'taxon_family', 'taxon_family_slug', 'taxon_url',
...
class TaxonSpeciesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'is_endemic', 'taxon_common_name',
                  ...
                  'taxon_genus_slug', 'taxon_url', 'taxon_family_slug',
                  'taxon_order_slug', 'taxon_class_slug', 'taxon_phylum_slug',
                  'taxon_kingdom_slug', 'taxon_domain_slug',
                  'created_by', 'created_datetime', 'modified_datetime', ]
    # Since project, system, watershed, and created_by reference different tables and we
...
class TaxonomicAnnotationSerializer(serializers.ModelSerializer):
    ...
    manual_domain = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_domain_slug', queryset=TaxonDomain.objects.all())
    manual_kingdom = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_kingdom_slug', queryset=TaxonKingdom.objects.all())
    manual_phylum = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_phylum_slug', queryset=TaxonPhylum.objects.all())
    manual_class = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_class_slug', queryset=TaxonClass.objects.all())
    manual_order = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_order_slug', queryset=TaxonOrder.objects.all())
    manual_family = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_family_slug', queryset=TaxonFamily.objects.all())
    manual_genus = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_genus_slug', queryset=TaxonGenus.objects.all())
    manual_species = serializers.SlugRelatedField(many=False, allow_blank=True, read_only=False, slug_field='taxon_species_slug', queryset=TaxonSpecies.objects.all())
