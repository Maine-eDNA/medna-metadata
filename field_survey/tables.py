...
class FieldSurveyTable(tables.Table):
    _selected_action = tables.CheckBoxColumn(accessor="pk",
                                             ...
                                             orderable=False)
    project_ids = tables.ManyToManyColumn(accessor='project_ids.project_label')
    supervisor = tables.Column(accessor='supervisor.agol_username')
    username = tables.Column(accessor='username.agol_username')
    survey_datetime = tables.DateTimeColumn(format="M d, Y")
    crew_full_name = tables.Column(accessor='field_crew.crew_full_name')
    arrival_datetime = tables.DateTimeColumn(format="M d, Y")
    site_id = tables.Column(accessor='site_id.site_id')
    env_measurements = tables.Column(accessor='env_measurements.env_measure_type_label')
