...
class SerializerExportMixin(ExportMixin):
    ...
    def get_table_data(self):
        ...
        if selected_column_ids:
            ...
            logging.info('SerializerExportMixin: get_table_data: ["{}"].'.format(super().get_table_data().objects.filter(pk__in=selected_column_ids)))
            return super().get_table_data().objects.filter(pk__in=selected_column_ids)
