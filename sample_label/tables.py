...
class SampleLabelRequestTable(tables.Table):
    ...
    max_sample_label_id = tables.LinkColumn(urlconf='detail_samplelabelrequest',
                                            args=[A('pk')],
                                            attrs={"th": {"class": "text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 field-max_sample_label_id"}})
    # Change column header
    min_sample_label_num = tables.Column(verbose_name="Min Label Num",
                                         attrs={"th": {"class": "text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 field-min_sample_label_num"}})
    ...
    add_label = tables.LinkColumn(urlconf="add_samplelabelrequestdetail",
